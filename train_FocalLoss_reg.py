...
from lossfunc.FocalLoss_reg import FocalLoss
...
def loss_calculate(pre, label, opt):
    ...
    elif opt.loss_func == "L2_Hinge_loss":
        l2_hinge_loss = torch.max(F.mse_loss(pre, label) - torch.tensor(25), torch.tensor(0))
        return l2_hinge_loss

    elif opt.loss_func == "FocalLoss":
        criterion = FocalLoss(gamma=2, alpha=1, reduction='mean')
        focal_loss = criterion(pre, label)
        return focal_loss

    else:
        raise ValueError("your loss function is not exist.")
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--describe", type=str, default='FocalLoss', help="describe for this model")
    ...
    parser.add_argument('--loss_func', type=str, default="FocalLoss",
                        choices=('SmoothL1Loss', 'mse', 'bp_bucketing_loss', 'HuberLoss', 'L2+Hinge', "FocalLoss"),
